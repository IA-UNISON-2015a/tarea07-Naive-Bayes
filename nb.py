...
class NaiveBayes:
    ...
    def aprende(self, datos, clases):
        ...
        for clase in clases:
            ...
            for var in self.var_nom:
                for val in self.vals[var]:
                    self.log_probs[var][clase][val] = log(
                        (self.frec[var][clase][val]+1) / 
                        (len(self.vals[var])+self.frec['clases'][clase])
                    )
        ...
        for dato in datos:
            ...
            for clase in self.clases:
                prob_clase.append((
                    ...
                    sum([self.log_probs[var][clase][val]
                        for (var, val) in zip(self.var_nom, dato)]),
                    clase
                ))    
            # print("prob_class: {}".format(prob_clase))
